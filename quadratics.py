...
def glovers_linearization(quad, bounds="tight", constraints="original", lhs_constraints=False, use_diagonal=False, solve_continuous=False, **kwargs):
	...
	if solve_continuous:
		x = m.binary_var_list(n, name="decision_var")
	else:
	...
	def reorder_refinedtest(self, k, take_max=False, flip_order=False):
		...
		for iter in range(k):
			...
			if type(self) is Knapsack: #HSP and UQP don't have cap constraint
				...
				#k_item constraint if necessary (if KQKP or HSP)
				if self.num_items > 0:
					m.add_constraint(m.sum(x[i] for i in range(n)) == self.num_items)
			...
			for i in range(n-1):
				for j in range(i+1, n):
					value_matrix[i,j] = self.C[i,j]*m.get_var_by_name("decision_var_"+str(i)).solution_value*m.get_var_by_name("decision_var_"+str(j)).solution_value
					if self.C[i,j] < 0:
