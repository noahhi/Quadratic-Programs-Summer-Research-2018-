...
#continuous versions of cplex glovers
def glovers_linearization(quad, bounds="tight", constraints="original", lhs_constraints=False, use_diagonal=False, **kwargs):
	...
	m = Model(name='continuous_relax_model')
...
def qsap_glovers(qsap, bounds="original", constraints="original", lhs_constraints=False, **kwargs):
	...
	x = mdl.continuous_var_matrix(keys1=m,keys2=n,name="decision_var", ub=1)
...
class Quadratic:  # base class for all quadratic problem types
	...
	def reorder_refined(self, k=5, alpha=0.3, take_max=False, flip_order=False):
		n = self.n

		# start by applying the original reorder method
		self.reorder(take_max=take_max, flip_order=flip_order)

		#stores the current and previous solutions to glovers relax
		sol = [np.zeros(n), np.zeros(n)]

		for iter in range(k):
			# model and solve continuous relax
			m = glovers_linearization(self, solve_continuous=True)[0]
			m.solve()
			#store solution
			for i in range(n):
				sol[iter%2][i] = m.get_var_by_name("decision_var_"+str(i)).solution_value

			#compute new xbar values based on current, previous solutions and alpha
			new_x = np.zeros(n)
			for i in range(n):
				curr_x_i = sol[iter%2][i]
				prev_x_i = sol[(iter+1)%2][i]
				new_x[i] = curr_x_i*alpha + (1-alpha)*prev_x_i

			value_matrix = np.zeros((n, n))
			for i in range(n-1):
				for j in range(i+1, n):
					value_matrix[i, j] = self.C[i, j]*new_x[i]*new_x[j]
					if self.C[i, j] < 0:
...
class UQP(Quadratic):  # unconstrained 0-1 quadratic program
	...
	def __init__(self, seed=0, n=10, m=1, density=100, symmetric=False):
		...
		for i in range(n):
			...
			for j in range(i+1, n):
				self.C[i, j] = -(2*Q[i, j])
				# NOTE symmetric for glovers only
				if symmetric:
...
class QSAP:	 # quadratic semi assignment problem
	...
	def reorder_refined(self, k=0.5, alpha=0.3, take_max=False, flip_order=False):
		n = self.n
		m = self.m
		...
		for iter in range(k):
			...
			for i in range(m):
				for k in range(n):
					sol[iter%2][i,k] = mdl.get_var_by_name("decision_var_"+str(i)+"_"+str(k)).solution_value

			#compute new xbar values based on current, previous solutions and alpha
			new_x = np.zeros((m,n))
			for i in range(m):
				for k in range(n):
					curr_x_ik = sol[iter%2][i,k]
					prev_x_ik = sol[(iter+1)%2][i,k]
					new_x[i,k] = curr_x_ik*alpha + (1-alpha)*prev_x_ik
				...
				for k in range(n):
					for j in range(i+1, m):
						for l in range(n):
							...
							value_matrix[i,k,j,l] = self.c[i,k,j,l]*new_x[i,k]*new_x[j,l]
							if self.c[i,k,j,l] < 0:
