...
def performance_profile(df, save_loc, variable, formulations, look_at="instance_total_time"):
	...
	writer = pd.ExcelWriter(
		save_loc+"perf_profile_report.xlsx", engine='xlsxwriter')
	df.to_excel(writer, index=False, sheet_name='Sheet1')
	writer.save()
	...
	for f in range(nf):
		xf = r[:, f]
		plt.plot(xf, yf, drawstyle="steps",
	...
	# can set xmax to zoom in/out #TODO make xmax a variable. or base automatically on ratio?
...
def make_bar_graph(df, save_loc, variable, formulations, look_at="instance_total_time"):
	...
	for form in formulations:
		...
		plt.text(bar.get_x()+bar.get_width()/2, height,
				 num_nans, ha='center', va='bottom')

	plt.title("Solve Time Comparison for {}".format(variable))
	plt.xlabel("Formulation")
	plt.ylabel("Average Solve Time")
	plt.legend([form for form in formulations])
	# format="pdf" to save as pdf (default is png)
	plt.savefig(save_loc+"bar_graph_"+variable)
	plt.close()
...
def make_line_graph(dfs, save_loc, variable, formulations):
	...
	for form in formulations:
		...
		plt.plot(range(len(dfs)), avg_times, label=str(
			form)+" : "+str(sum(num_nans))+" time limit")
...
def helpful_snippets():
	...
	data = pd.read_excel(
		open("C:/Users/huntisan/Desktop/summer2018/std_glove_data.xlsx", "rb"))
	...
	if(len(sys.argv) == 5):	 # batch file will go through here
		run_trials(trials=5, type=sys.argv[1], method=sys.argv[2], den=int(
			sys.argv[3]), size=int(sys.argv[4]))


"""
INSTRUCTION FOR CALLS TO ANALYZE:

test_variable options: 'solver', 'method', 'glover_bounds', 'options' (ie. any column headers from report)
